...
def testtempsfeu (lattice,n):         #teste si le graphe est connexe
    tempsdarrivee=[0]   #temps pour atteindre la k-eme ligne
    tempsactuel=0
    lignesuivante=1
    positions=[(0,k) for k in range (n)]
    for site in positions:
    ...
    while positions!=[]:
        ...
        positions=suivants
        for k in positions:
            if k[0]==lignesuivante:
                tempsdarrivee.append(tempsactuel)
                lignesuivante+=1
            if lignesuivante==n:
                return tempsdarrivee
    return tempsdarrivee
    
    
def stopageidealfeu(M, n,p,cc):   #cc: capacité de coupe= nombres d'arbres qu'on peut couper par tour (temps majorée par (n+1)//cc)
    """tester sur la matrice le coupage à toutes les lignes en partant de 1"""
    L=testtempsfeu(M,n)
    for k in range (1,n//cc):  #à chaque ligne on compte le bombre d'arbres à couper, et on regarde si on avait le temps de couper
        nbarbresdelaligne=0
        for i in range (len(L)):
            if M[k,i]==-2:  #nbpour le feu
                nbarbresdelaligne+=1
        if k>=len(L):
            return len(L)+1

        if (nbarbresdelaligne//cc)<L[k]:
            return k
    return len(L)+1

#fixons n et cc. on prend n grand (ex n=1000, et on pose cc==int(n²p/(1400 000 000//244 900))  #arbres//pompiers
#1ex: pour n=1000 et p=0.5, cc =87
#on a donc

def frontdebloquagemoyendufeu(n):
    P = linspace(0, 1, 41)
    F=[]
    for p in P:
        M=lattice (n, p)
        cc=int((n**2)/(1400000000/244900))
        F.append(stopageidealfeu(M, n,p,cc))
    clf()
    plot(P,F)
    show()